...
local function initInterface()
  ...
  checkAutoBuy = marketWindow:recursiveGetChildById('autoBuy')
...
function init()
  ...
  refreshEvent = cycleEvent(Market.procNextItem, 8000)
end
...
function Market.onMarketDetail(itemId, descriptions, purchaseStats, saleStats)
  ...
  if checkAutoBuy:isChecked() then
    local row = sellOfferTable:getRow(0)
    sellOfferTable:selectRow(row)
  end
...
function Market.onMarketBrowse(offers)
  --[[ BOT ]]
  if checkAutoBuy:isChecked() then
    local lowerPrice = 10000000000
    local currOffer = nil
    for i = 1, #offers do
        local tmpOffer = offers[i]
        if tmpOffer:getPrice() < lowerPrice and tmpOffer:getType() ~= MarketAction.Buy then
          lowerPrice = tmpOffer:getPrice()
          currOffer = offers[i]
        end
    end

    if currOffer ~= nil then
      Market.checkOffer(currOffer)
    end
    currIndexMkt = currIndexMkt+1
    startNextItem = true
  end
...
--[[ BOT ]]
function Market.checkOffer(offer)
    local price = offer:getPrice()
    local amount = offer:getAmount()
    local timestamp = offer:getTimeStamp()
    local itemName = offer:getItem():getMarketData().name
...
function Market.procNextItem()
  if startNextItem == false then return end
  if marketWindow:isHidden() then return end
  if checkAutoBuy:isChecked() == false then return end

  if currIndexMkt > countTableItensToBuy() then currIndexMkt = 0 end
  startNextItem = false
...
function Market.selectItem(browseId)
  clearMyOffers()
